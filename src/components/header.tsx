  ...
  return (
    <header className="z-[999] relative">
      ...
      <nav className=" flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
        <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-sm font-thin text-white sm:w-[initial] sm:flex-nowrap sm:gap-5">
          {links.map((link) => (
            <motion.li
              key={link.link}
            ...
            >
              <Link
                className={clsx(
                  ...
                  {
                    "text-gray-950 font-normal dark:text-gray-200":
              ...
              >
                {link.name && <div className="">{link.name}</div>}
